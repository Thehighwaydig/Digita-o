...
function init() {
    ...
    phraseWords = textToType.split(/\s+/);
...
typingInput.addEventListener('input', () => {
    ...
    // --- NOVA LÓGICA DE VOZ: Falar ao começar a palavra ---
    // Contamos quantos espaços existem antes do cursor para saber o índice da palavra atual
    const wordsTypedCount = inputVal.split(' ').length - 1;
    
    // Se o índice da palavra atual for maior que o último falado, falamos a nova palavra
    if (wordsTypedCount > lastSpokenWordIndex) {
        const wordToSpeak = phraseWords[wordsTypedCount];
        if (wordToSpeak) {
            speak(wordToSpeak, 1.1);
            lastSpokenWordIndex = wordsTypedCount;
        }
    }
